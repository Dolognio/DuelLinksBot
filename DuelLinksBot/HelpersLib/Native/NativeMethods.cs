...
namespace DuelLinksBot.HelpersLib.Native
{
	...
	internal static partial class NativeMethods
	{
		...
		[DllImport("user32.dll")]
		internal static extern IntPtr DefWindowProc(IntPtr hWnd, uint uMsg, UIntPtr wParam, IntPtr lParam);
		...
		///  If the specified window is a topmost window, the function retrieves a handle to the next (or previous) topmost
		///  window.
		///  If the specified window is a top-level window, the function retrieves a handle to the next (or previous) top-level
		///  window.
		///  If the specified window is a child window, the function searches for a handle to the next (or previous) child
		///  window.
		/// </summary>
		[DllImport("user32.dll")]
		internal static extern IntPtr GetWindow(IntPtr hWnd, GetWindowConstants wCmd);
		...
		[DllImport("user32.dll")]
		internal static extern IntPtr LoadCursor(IntPtr hInstance, int iconId);
		...
		[DllImport("user32.dll")]
		internal static extern bool ReleaseCapture(IntPtr hwnd);
		...
		[DllImport("user32.dll")]
		internal static extern IntPtr SendMessage(IntPtr hWnd, int Msg, int wParam,
		...
		[DllImport("user32.dll", CharSet = CharSet.Auto)]
		internal static extern IntPtr SendMessageTimeout(IntPtr hWnd, int Msg, int wParam, int lParam,
		...
		internal static extern bool SetProcessWorkingSetSize(IntPtr handle, IntPtr min, IntPtr max);
		...
		[DllImport("gdi32.dll")]
		internal static extern IntPtr CreateRoundRectRgn(int nLeftRect, int nTopRect, int nReghtRect, int nBottomRect,
		...
		[DllImport("gdi32.dll")]
		internal static extern IntPtr CreateDIBSection(IntPtr hdc, [In] ref BITMAPINFOHEADER pbmi, uint pila,
		...
		[DllImport("dwmapi.dll")]
		internal static extern int DwmGetWindowAttribute(IntPtr hwnd, int dwAttribute, out bool pvAttribute,
			int cbAttribute);


		[DllImport("dwmapi.dll")]
		internal static extern int DwmGetWindowAttribute(IntPtr hwnd, int dwAttribute, out int pvAttribute,
			int cbAttribute);
		...
		[DllImport("dwmapi.dll")]
		internal static extern int DwmSetDxFrameDuration(IntPtr hwnd, uint cRefreshes);
		...
		internal static extern int memcpy(int dst, int src, int count);
		...
		internal static extern int AVIFileOpen(out IntPtr aviHandler, string fileName, OpenFileMode mode, IntPtr handler);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVIFileRelease(IntPtr aviHandler);
		...
		/// <param name="streamNumner">
		///  Count of the stream type. Identifies which occurrence of the specified stream type to
		///  access.
		/// </param>
		/// <returns></returns>
		[DllImport("avifil32.dll")]
		internal static extern int AVIFileGetStream(IntPtr aviHandler, out IntPtr streamHandler, int streamType,
			int streamNumner);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVIStreamRelease(IntPtr streamHandler);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVIStreamSetFormat(IntPtr streamHandler, int position, ref BITMAPINFOHEADER format,
			int formatSize);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVIStreamStart(IntPtr streamHandler);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVIStreamLength(IntPtr streamHandler);
		...
		/// <param name="wantedFormat">
		///  Pointer to a structure that defines the desired video format. Specify NULL to use a default
		///  format.
		/// </param>
		/// <returns>Returns an object that can be used with the <see cref="AVIStreamGetFrame" /> function.</returns>
		[DllImport("avifil32.dll")]
		internal static extern IntPtr AVIStreamGetFrameOpen(IntPtr streamHandler, ref BITMAPINFOHEADER wantedFormat);
		...
		[DllImport("avifil32.dll")]
		internal static extern IntPtr AVIStreamGetFrameOpen(IntPtr streamHandler, int wantedFormat);
		...
		/// <returns>Returns zero if successful or an error otherwise.</returns>
		[DllImport("avifil32.dll")]
		internal static extern int AVIStreamGetFrameClose(IntPtr getFrameObject);
		...
		[DllImport("avifil32.dll")]
		internal static extern IntPtr AVIStreamGetFrame(IntPtr getFrameObject, int position);
		...
		/// <returns>Returns zero if successful or an error otherwise.</returns>
		[DllImport("avifil32.dll")]
		internal static extern int AVIStreamWrite(IntPtr streamHandler, int start, int samples, IntPtr buffer,
			int bufferSize, int flags, IntPtr samplesWritten,
			IntPtr bytesWritten);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVISaveOptions(IntPtr window, int flags, int streams,
			[In] [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)]
			IntPtr[] streamInterfaces,
			[In] [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)]
			IntPtr[] options);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVISaveOptionsFree(int streams,
			[In] [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)]
			IntPtr[] options);
		...
		[DllImport("avifil32.dll")]
		internal static extern int AVIMakeCompressedStream(out IntPtr compressedStream, IntPtr sourceStream,
			ref AVICOMPRESSOPTIONS options, IntPtr clsidHandler);
